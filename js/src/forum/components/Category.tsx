...
export default class Category extends Component<Attrs> {
  ...
  lockedItems() {
    ...
    items.add('LockedText', <div className={classList('TagCategory-lockedText')}>{this.tagLocked.text}</div>, 50);
    return items;
  }
    ...
    if (this.isChild || !app.forum.attribute('categories.parentRemoveStats')) {
      items.add(
        ...
        StatWidget.component({
          ...
          label: app.translator.trans('fof-categories.forum.stat-widgets.discussion_label'),
      ...
      items.add(
        ...
        StatWidget.component({
          ...
          label: app.translator.trans('fof-categories.forum.stat-widgets.post_label'),
          icon: 'fas fa-comment',
        }),
        10
      );
